...
func (p *Provider) PushBranchToRemote(branchName string) error {
	...
	p.logger.Info().Msgf("Branch %s pushed to remote successfully by user %s", branchName, username)
	return nil
}
...
// getGitUsername retrieves the Git username from the local configuration.
