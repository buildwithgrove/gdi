...
func RunFirstTimeSetup() error {
	...
	fmt.Print(log.Green + "❓ Would you like to configure git configuration?" + log.ResetColor + "\nThis is only necessary for private repositories. It must be a valid PAT with at least `write:repo` scope." + log.Blue + "\nYou may edit this choice later using 'gdi config'." + log.ResetColor + "\n(y/n): ")
	answer, _ = reader.ReadString('\n')
	answer = strings.TrimSpace(strings.ToLower(answer))
	cfgEditor.ClearTerminal()
	if answer == "y" {
